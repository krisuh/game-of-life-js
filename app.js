(function() {
        ...
        for(var y = 0; y < yLength; y++) {
            ...
            for(var x = 0; x < xLength; x++) {
                ...
                cell.addEventListener('click', function(e) {
                    ...
                    var styleStr = this.alive ? 'background: black;' : 'background: white;';
    ...
    function setAdjacentIndices(x, y, limitY, limitX) {
        west = {name: 'west', x: x - 1, y: y};
        east = {name: 'east', x: x + 1, y: y};
        north = {name: 'north', x: x, y: y - 1};
        south = {name: 'south', x: x, y: y + 1};
        if(west.x < 0) {
            west.x = limitX;
        }
        if(east.x > limitX) {
        ...
        northEast = {name: 'northEast', x: east.x, y: north.y};
        northWest = {name: 'northWest', x: west.x, y: north.y};
        southEast = {name: 'southEast', x: east.x, y: south.y};
        southWest = {name: 'southWest', x: west.x, y: south.y};
